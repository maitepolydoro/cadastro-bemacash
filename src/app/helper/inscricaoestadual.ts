...
export function validar(ie: string | Array<string>, estado: any):boolean {
  ...
  if (estado !== '' && !(estado in funcoes)) {
    return false//new Error('estado não é válido');
  }

  if (eIndefinido(ie)) {
    return false//new Error('ie deve ser fornecida');
  }
  ...
  if (typeof ie !== 'string') {
    return false //new Error('ie deve ser string ou array de strings');
  }

  if (!allNumbersAreSame(ie)) {
    return false //new Error('ie com todos dígitos iguais');
